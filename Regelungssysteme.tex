...
% Document Class ===============================================================
% mathtools must be loaded before the class: latex4ei_sheet opens the multicols*
% environment in \AtBeginDocument, and mathtools' own begin-document hook would
% then load a package inside that group.
...
\mdfapptodefinestyle{sectionbox}{skipabove=0.5mm, skipbelow=0.5mm,
	innertopmargin=1mm, innerbottommargin=1mm}
\mdfapptodefinestyle{tablebox}{skipabove=0.5mm, skipbelow=0.5mm,
	startcode={}, endcode={}, innertopmargin=1mm, innerbottommargin=1mm}
\mdfapptodefinestyle{emphbox}{skipabove=0.5mm, skipbelow=0.5mm,
	startcode={}, endcode={}, innertopmargin=1mm, innerbottommargin=1mm}
\mdfapptodefinestyle{symbolbox}{skipabove=0.5mm, skipbelow=0.5mm,
	startcode={}, endcode={}, innertopmargin=1mm, innerbottommargin=1mm}
\mdfapptodefinestyle{bluebox}{skipabove=0.5mm, skipbelow=0.5mm,
	startcode={}, endcode={}, innertopmargin=1mm, innerbottommargin=1mm}

% set document information
\title{Regelungssysteme}

% \Fbox was provided by the old latex4ei class
...
\begin{sectionbox}\subsection{Zustandsbeschreibung linearer Systeme}
...
\begin{sectionbox}
	...
	\begin{emphbox} $\displaystyle G(s) = \frac{Y(s)}{U(s)} = \frac{\beta_m s^m + \ldots + \beta_1 s + \beta_0}{\alpha_n s^n + \ldots + \alpha_1 s + \alpha _0} = \frac{Z(s)}{N(s)}$ \end{emphbox}
	($n =$ Ordnung der DGL $=$ Anzahl der Pole)\\
	\begin{emphbox}Übertragungsfunktion der Zustandsbeschreibung: \\$\ma G (s) = \eset{\ma C (s \ma E - \ma A)^{-1} \ma B + \ma D}$\end{emphbox}
...
\begin{itemize}
	...
	\item Regelungsnormalform: \\
		...
		$\vec s_R^\top = \mat{0 & \dots & 0 & 1} \cdot \ma S_S^{-1}$

		! RNF existiert nur falls $\ma S_S$ regulär ist $\ra $ System ist vollst. steuerbar

		Transformationsmatrix $\ma T_R = \mat{ \vec s_R^T \\ \vec s_R^T \ma A \\ \vdots \\ \vec s_R^T \ma A^{n-1}}^{-1}$
...
\begin{sectionbox}
	...
	\begin{emphbox} $\lim\limits_{\mathclap{\phantom{:} t \ra 0^+}} \; x(t) = \lim\limits_{\strut \mathclap{s \ra \infty}} \; s X(s)$ \qquad\qquad $\lim\limits_{\strut \mathclap{t \ra \infty}} \; x(t) = \lim\limits_{\mathclap{\phantom{:} s \ra 0^+}} \; s X(s)$ \end{emphbox}
...
Es gilt: $\textbf{zustandsstabil} \rightarrow \textbf{E/A stabil}$
\end{sectionbox}

\begin{sectionbox}
\subsubsection{Stabilitätsbedinung für LTI-Systeme}
\begin{emphbox}$\Re{\lambda_i (A)} < 0 \quad i = 1, \ldots, n$\end{emphbox}
\end{sectionbox}
...
\subsection{Direkte Methode von Lyapunov}
	...
	\begin{cookbox}{Direkte Methode von Lyapunov für lineare Systeme}
...
%Systemmatrix $\ma A$: Alle EW $\lambda_i < 0$\\
	...
	\subsubsection{Pole}
	Pole $p_i \subseteq \lambda_i$: Alle $\Re{p_i} < 0$ \\
...
\subsection{Zustandssteuerbarkeit und -beobachtbarkeit}
	...
	\subsubsection{Zustandsbeobachtbarkeit}
		Def.: Anfangszustände $\vec x_0$ aus Verlauf $\vec y(t < \infty)=\ma C \vec x$ bestimmbar \\\\
	...
	Falls \textbf{steuerbar} und \textbf{beobachtbar}: $p_i = \lambda_i$
\end{sectionbox}
...
Die Auswirkungen auf Amplitude $A$ und Phasenverschiebung $\varphi$ ergeben die Frequenzgangfunktion $G(\j \omega)$
	...
	\begin{emphbox}$G(\j \omega) = A(\omega) e^{\j \varphi (\omega)} = \Re {G(\j \omega)} + \j \Im{G(\j \omega)}$ \end{emphbox}
...
\begin{sectionbox}
	...
	\begin{emphbox}$G_0 (\j \omega) \overset{!}{=} -1 + \i 0$\end{emphbox}
...
\begin{sectionbox}
	...
	\end{enumerate}
	\begin{emphbox}$W_{\ir ist} = \mathop{\Delta}\limits_{\omega = 0}^{\omega = \infty} \Phi \stackrel{!}{=} W_{\ir soll} = \pi n_{\ir rechts} + \frac{\pi}{2} n_{\ir auf}$\end{emphbox}
...
\begin{sectionbox}
	...
	anwendbar falls $n_{\ir rechts} = 0$ und $n_{\ir auf} \le 1$ und $G_0(s)$ gebrochen rational\\
...
\begin{sectionbox}
	...
	\begin{emphbox}$G (\j \omega) = A(\omega) e^{\j \varphi (\omega)}$\end{emphbox}
...
\begin{sectionbox}
	...
	\begin{emphbox}$G_0 (s; K) = K \frac{Z_0 (s)}{N_0 (s) } = KQ \frac{\prod_{\mu = 1}^{m} (s-q_\mu^0)}{\prod_{\nu = 1}^n (s-p_\nu^0)}$\end{emphbox}
	...
	\begin{itemize}
		...
		ist.
	\end{itemize}
	Geschlossener Regelkreis: $N_{\ir geschlossen}=N_0(s)+KZ_0(s)$ \\
	Verzweigungspunkt: $N_{\ir RK}(s) = 0 \land \frac{\diff}{\diff s}N_{\ir RK}(s) = 0$\\
	...
	{stabil}: $\Re{p_i} < 0$\\
	{schwingfähig}: $\Im{p_i} \neq 0$\\
	{sprungfähig}: $\#NS = \#PS$, $D=0$\\
	{kausal/real-implementierbar}: $\#NS < \#PS$, $D=0$
\end{sectionbox}
...
\begin{sectionbox}
	\subsection{Zustandsbeobachter (Simulation)}
		...
		\begin{emphbox}$\dot{\hat{\vec x}} = \underbrace{(\ma A - \ma L \ma C)}_{\hat{\ma A}  = \ma{A}_{\ir Beo}} \hat{\vec x}  + \ma B \vec u + \ma L \vec y$ \quad \quad
		$\hat{\vec y} = \ma C \hat{\vec x}$\end{emphbox}
...
$a_n y_{k-n} + a_{n-1} y_{k-n+1} + \ldots + a_0 y_k = b_m u_{k-m} + \ldots + b_0 u_k$
	...
	\\
	\begin{emphbox} $\displaystyle H(z) = \frac{Y(z)}{U(z)} = \frac{\beta_r z^r + \ldots + \beta_1 z + \beta_0}{\alpha_n z^n + \ldots + \alpha_1 z + \alpha_0}$ \end{emphbox}
...
% Spalten werden von latex4ei_sheet automatisch beendet
